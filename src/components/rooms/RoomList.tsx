  ...
  const load = async () => {
    ...
    try {
      const data = await fetchRooms({
        ...
        minCapacity:
          typeof filters.minCapacity === "number" ? filters.minCapacity : undefined,
      });
      setItems(data);
    } finally {
      setLoading(false);
    ...
    <Box>
      <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
        <TextField
          ...
          onChange={(e) => setFilters((f) => ({ ...f, buildingName: e.target.value }))}
        />
        <TextField
          label="층"
          size="small"
      ...
      <RoomCreateDialog
        ...
        editRoomId={editRoomId ?? undefined} // ✅ RoomCreateDialog가 기대하는 prop 이름에 맞춤
      />
