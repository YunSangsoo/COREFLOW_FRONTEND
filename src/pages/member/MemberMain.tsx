...
export default function MemberMain() {
    ...
    */
    ...
    const {data:departmentDetail} = useQuery<DepartmentDetail[]>({
        queryKey : ['departmentDetails', searchValues.parentId],
        queryFn : () => depDetailList(searchValues.parentId!),
        enabled : searchValues.parentId !== null
    })
    ...
    const handleSearch = () => {
        ...
        const selectedParentDep = department?.find(dep => dep.depId === searchValues.depId);
        const selectedChildDep = departmentDetail?.find(dep => dep.depId === searchValues.parentId);

        const finalDepName = selectedChildDep ? selectedChildDep.depName : (selectedParentDep ? selectedParentDep.depName : null);
        
        setSearchParams({
            userName: searchValues.userName || null,
            depName: finalDepName,
            posName: searchValues.posName || null,
        });
    };
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.searchSection}>
                ...
                <span>부서
                    <select name="parentDepId" value={searchValues.depId || ''} onChange={handleChange}>
                ...
                {/* 부모 부서가 선택되었을 때만 자식 부서 셀렉트 박스 표시 */}
                {searchValues.depId && (
                    <span>
                        <select name="childDepId" value={searchValues.parentId || ''} onChange={handleChange}>
